...
export default class BeaconMonitoringAndRanging extends Component {
	...
	componentDidMount() {
		...
		this.regionDidEnterEvent = Beacons.BeaconsEventEmitter.addListener(
			...
			data => {
				...
				this.setState({
					regionEnterDatasource: this.state.rangingDataSource.cloneWithRows(
						[
							{
		...
		this.regionDidExitEvent = Beacons.BeaconsEventEmitter.addListener(
			...
			({ identifier, uuid, minor, major }) => {
				...
				const time = moment().format(TIME_FORMAT)
				this.setState({
					regionExitDatasource: this.state.rangingDataSource.cloneWithRows(
	...
	render() {
		const {
			// bluetoothState,
			regionEnterDatasource,
			regionExitDatasource,
		} = this.state
	...
	renderMonitoringEnterRow = ({ identifier, uuid, minor, major, time }) => {
		return (
			<View style={styles.row}>
				...
				<Text style={styles.smallText}>time: {time ? time : 'NA'}</Text>
			</View>
		)
	}
